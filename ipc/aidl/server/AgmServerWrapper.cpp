...
void SessionInfo::dump() {
    ALOGV("session Id %d handle %lx, aif size %zu", mSessionId, mHandle, mAifIds.size());
}

SessionInfo::~SessionInfo() {
    ALOGV("%s session Id %d handle %lx, noOfAifs %zu fdPairs %zu", __func__, mSessionId, mHandle,
...
void SessionInfo::forceCloseSession() {
    ...
    if (mHandle) {
        ALOGV("force closing session with handle %lx", mHandle);
...
void ClientInfo::addSessionHandle(uint32_t sessionId, uint64_t handle) {
    ...
    ALOGI("%s sessionId %d handle %lx ", __func__, sessionId, handle);
}

void ClientInfo::removeSessionHandle(uint64_t handle) {
    std::lock_guard<std::mutex> guard(mSessionLock);
    ALOGV("%s,  removeSessionhandle %lx in session of size %zu ", __func__, handle,
    ...
    for (; itr != mSessionsInfoMap.end();) {
        ...
        if (handle == sessionInfo->getHandle()) {
            ALOGI("%s removing handle %lx for session %u", __func__, handle, itr->first);
    ...
    ALOGV("%s, Exit: removeSessionhandle %lx in session of size %zu ", __func__, handle,
          mSessionsInfoMap.size());
}
...
void ClientInfo::addSharedMemoryFdPairs(uint64_t handle, int inputFd, int dupFd) {
    std::lock_guard<std::mutex> guard(mSessionLock);
    ALOGV("%s handle %lx, inputFd %d dupFd %d sessionSize %zu", __func__, handle, inputFd, dupFd,
          mSessionsInfoMap.size());
...
::ndk::ScopedAStatus AgmServerWrapper::ipc_agm_session_open(int32_t in_sessionId,
    ...
    *_aidl_return = static_cast<int64_t>(handle); // TODO;
    ...
    ALOGV("%s handle received is : %llx, ret %d, rethandle %lx", __func__,
...
::ndk::ScopedAStatus AgmServerWrapper::ipc_agm_session_write(int64_t in_handle,
    ...
    ALOGV("%s called with handle = %llx count = %zu",
        __func__, (unsigned long long)in_handle, count);
...
::ndk::ScopedAStatus AgmServerWrapper::ipc_agm_aif_set_params(
    ...
    ALOGV("%s %d aifId %zu size %d", __func__, in_aifId, in_payload.size());
...
::ndk::ScopedAStatus AgmServerWrapper::ipc_agm_get_hw_processed_buff_cnt(int64_t in_handle,
    ...
    ALOGV("%s handle %lx", __func__, in_handle);
